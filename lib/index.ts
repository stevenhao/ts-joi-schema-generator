...
export async function main() {
  commander
    ...
    .option(
      "-s, --schema-suffix <postfix>",
      `Postfix to append to generated Joi schemas (default ${Defaults.schemaSuffix})`,
      Defaults.schemaSuffix
    )
    .option(
      "-f, --file-suffix <suffix>",
      `Suffix to append to generated files (default ${Defaults.fileSuffix})`,
      Defaults.fileSuffix
    )
    .option(
      "-o, --outDir <path>",
      "Directory for output files; same as source file if omitted"
    )
    .allowUnknownOption()
    .parse(process.argv);
  ...
  const options: ICompilerOptions = {
    ...
    tscArgs: [
      ...(commander.project ? ["-p", commander.project] : []),
      ...tscArgs,
    ],
  };
  ...
  await Promise.all(
    result.map((compileResult) =>
      fs.outputFile(compileResult.schemaFile, compileResult.content)
    )
  );
  console.log(result);
}
